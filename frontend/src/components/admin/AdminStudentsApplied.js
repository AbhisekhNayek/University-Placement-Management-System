...
const AdminStudentsApplied = () => {
    ...
    const handleDelete = (appliedid) => {
        fetch(`http://localhost:1337/api/deleteCandidate/${appliedid}`, {method: 'DELETE'})
            .then((response) => response.json())
            .then((data) => {
                if (data.success) {
    ...
    return (
        <div className=' h-screen w-full'>
            ...
            <div className=" font-robotoMono font-bold text-center text-2xl">Students Applied</div>
                    ...
                    <Table>
                        <TableHead>
                            ...
                            sx={{
                                ...
                                {['ID', 'Student Name', 'Email', 'USN', 'Job Role', 'Company Name', 'Job Id', 'Status', 'Resume', 'Delete'].map((cell, indx) => (
                                    <TableCell
                                    sx={{
                                        fontSize: '18px',
                                        fontWeight: 'bold',
                        ...
                        <TableBody sx={{ color: 'black' }}>
                            {postingData?.map((posting) => (
                                <TableRow key={posting.appliedid}>
                                    <TableCell>{posting.appliedid}</TableCell>
                                    <TableCell>{posting.studentName}</TableCell>
                                    <TableCell>{posting.studentEmail}</TableCell>
                                    <TableCell>{posting.usn}</TableCell>
                                    <TableCell>{posting.jobRole}</TableCell>
                                    <TableCell>{posting.companyName}</TableCell>
                                    <TableCell>{posting.jobId}</TableCell>
                                    ...
                                    <TableCell>
                                        ...
                                        variant="contained"
                                        color="secondary"
                                        onClick={() => handleDelete(posting.appliedid)}>
                                            Delete
                                        </Button>
                                    </TableCell>
                                </TableRow>
